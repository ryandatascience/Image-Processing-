...
{
	image src, tgt,tgt1,tgt2, hist_ori, hist_modi,hist1_ori, hist1_modi;
	FILE *fp;
	char str[MAXLEN];
	char outfile[MAXLEN];
	char *pch;
	...
	else if (strncasecmp(pch,"Sobel33",MAXLEN)==0) {
			/* Sobel */
			pch = strtok(NULL, " ");
			utility::Sobel33(src,tgt,tgt1,tgt2);
			tgt1.save("Sbel33_threshold.pgm");
			tgt2.save("Sbel33_tdirection.pgm");
		}
	else if (strncasecmp(pch,"dftLPF",MAXLEN)==0 && argc>=3) {
			/* dftLPF */
			pch = strtok(NULL, " ");
			utility::dftLPF(argv[3],argv[4],tgt);
			tgt.save("OpenCV_dft.pgm");
